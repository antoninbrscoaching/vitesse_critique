...
if st.button("Calculer VC"):
    ...
    if T2 == T1:
        ...
        if V_kmh <= 0 or not math.isfinite(V_kmh):
            ...
            min_per_km = 60.0 / V_kmh         # minutes par km (float)

            # conversion robuste en mm:ss
            ...
            # Graphique : distance & vitesse (échelle lisible)
            T_max = max(T1, T2) * 2.0
            T_plot = np.linspace(max(5.0, T_max * 0.01), T_max, 500)  # évite 0
            # curve lissée (évite explosion à T->0)
            D_plot = CS * T_plot + D_prime * (1.0 - np.exp(-T_plot / 500.0))
            V_plot = np.gradient(D_plot, T_plot) * 3.6  # km/h
...
# Tableau des intensités avec allure
import pandas as pd
import math

pourcentages = list(range(102, 132, 2))  # de 102% à 130% par pas de 2
rows = []

for p in pourcentages:
    vitesse = V_kmh * (p / 100.0)  # vitesse en km/h
    temps_limite = D_prime / ((vitesse / 3.6) - CS)  # temps limite en secondes

    if temps_limite > 0 and math.isfinite(temps_limite):
        # Conversion temps limite en mm:ss
        minutes_lim = int(temps_limite // 60)
        seconds_lim = int(temps_limite % 60)
        temps_format = f"{minutes_lim}:{seconds_lim:02d}"

        # Conversion vitesse en allure min/km
